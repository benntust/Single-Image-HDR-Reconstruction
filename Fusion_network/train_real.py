...
def train():
    ...
    # Initial weight
    Darker.load_state_dict(torch.load(os.path.join(config.weights_path, 'Darkerbest_23.90.pkl')))
    Brighter.load_state_dict(torch.load(os.path.join(config.weights_path, 'Brighterbest_23.90.pkl')))
    F.load_state_dict(torch.load(os.path.join(config.weights_path, 'SamNetbest_joint_23.90.pkl')))
    #init_weights(F,init_type='kaiming')   
    
    # Optimizers #
    F_optim = torch.optim.Adam(F.parameters(), lr=config.lr, betas=(0.9, 0.999))
    Darker_optim = torch.optim.Adam(Darker.parameters(), lr=config.lr/20, betas=(0.9, 0.999))
    Brighter_optim = torch.optim.Adam(Brighter.parameters(), lr=config.lr/20, betas=(0.9, 0.999))
    ...
    for epoch in range(config.num_epochs):
        ...
        for i, (ldr, hdr) in enumerate(train_loader):
            Darker.train()
            Brighter.train()
            F.train()
            # over-/under-exposed images
            ...
            pred_hdr = F(dark, normal, bright)
            loss = 2*mu2_loss(pred_hdr, hdr) + 2*mu_loss(pred_hdr, hdr)
            loss.backward()
            Darker_optim.step()
            Brighter_optim.step()
            F_optim.step()
            ...
            hdr_numpy = torch.squeeze(hdr,0)
            hdr_numpy = hdr_numpy.detach().cpu().numpy().transpose((1, 2, 0))
            train_psnr, train_ssim = psnr_tanh_norm_mu_tonemap(pred_hdr_numpy,hdr_numpy)
            PSNRs.append(train_psnr)
            SSIMs.append(train_ssim)
            ...
            if (i+1) % config.print_every == 0:
                ...
                print("SamNet | Epochs [{}/{}] | Iterations [{}/{}] | G Loss {:.4f} | PSNR {:.4f} | SSIM {:4f}"
                    .format(epoch+1, config.num_epochs, i+1, total_batch, np.average(losses), np.average(PSNRs), np.average(SSIMs)))
        ...
        if psnr>best_psnr:
            best_psnr = psnr
            torch.save(Darker.state_dict(), os.path.join(config.weights_path, 'Darkerbest_real_filter.pkl'))
            torch.save(Brighter.state_dict(), os.path.join(config.weights_path, 'Brighterbest_real_filter.pkl'))
            torch.save(F.state_dict(), os.path.join(config.weights_path, 'SamNetbest_joint_real_filter.pkl'))
...
if __name__ == '__main__':
    ...
    if(0):
        ...
        Brighter = AttU_Net().to(device)
        # Initial weight
        Darker.load_state_dict(torch.load(os.path.join(config.weights_path, 'Darkerbest_real.pkl')))
        Brighter.load_state_dict(torch.load(os.path.join(config.weights_path, 'Brighterbest_real.pkl')))
        F.load_state_dict(torch.load(os.path.join(config.weights_path, 'SamNetbest_joint_real.pkl')))
